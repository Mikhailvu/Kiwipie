...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") || 0 == commandlineArguments.count("freq")) {
    ...
    auto onImageReading{[&behavior](cluon::data::Envelope &&envelope)
      {
        auto imageReading = cluon::extractMessage<opendlv::logic::sensation::Point>(std::move(envelope));
          behavior.setImage(imageReading);
      }};
